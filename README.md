...
Instead of a monolithic application, we adopted a **Micro-Frontend Architecture**. This lightweight dashboard acts as a shell, consuming our two headless AI "Brain" servers (hosted on Hugging Face) using **Gradio Web Components**. This ensures zero-latency updates: if we improve the model backend, this dashboard updates instantly without redeployment.
...
## 🛠️ Architecture
The application is built using a **Serverless/Headless** approach:

* **Frontend (This Repo):** Pure HTML5/CSS3 with Gradio Web Components (`<gradio-app>`). Hosted on **Vercel**.
...
3.  **Maintainability:** We can fix a bug in the Resume logic without breaking the Research Agent interface.
...
1.  **Clone the repository:**
    ```bash
    git clone [https://github.com/YourUsername/sic-team013-frontend.git](https://github.com/YourUsername/sic-team013-frontend.git)
    ```
2.  **Open in VS Code.**
3.  **Run with Live Server:**
    Since this uses modern Web Components, it requires a local server context. Right-click `index.html` and select **"Open with Live Server"**.
